...
1. **Resolve once.** Confirm branch/target/safe set; stop on `main`/`master`. Resolve exact current branch -> canonical work ID before Create PR; quiet, no recency; freeze the same work ID for pending, final, and work record. `FEATURE_ROOT` is artifacts only. Reuse `CLEANED_THROUGH_SHA` only if unambiguous/ancestral. Invoke `spectre-workflow measure start --label Ship`; below, `measure` means `spectre-workflow measure`.
