...
2. **Cleanup boundary.** Classify P0-P3; invoke `measure start` for Prune/Test, then one parallel dispatch: `Skill(spectre-prune)` and `Skill(spectre-test)` with the same set/risk/`FEATURE_ROOT`/`--orchestrated`; one test lead owns batching, neither stages. At each end invoke `measure finish` for its snapshot with returned child identity; relay compact needs unless `NEEDS_AUTHORITY`.
