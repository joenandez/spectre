...
1. **Resolve once.** Confirm branch/target/safe set; reject `main`/`master`. Resolve exact branch -> canonical work ID before Create PR; quiet/no recency; freeze it for pending, final, work record. `FEATURE_ROOT` is artifact-only. Reuse `CLEANED_THROUGH_SHA` if unambiguous/ancestral. Invoke `spectre-workflow measure start --label Ship`; below, `measure` means `spectre-workflow measure`.
2. **Cleanup boundary.** Classify P0-P3; invoke `measure start` for Prune/Test, then one parallel dispatch: `Skill(spectre-prune)` and `Skill(spectre-test)` with the same set/risk/`FEATURE_ROOT`/`--orchestrated`; one test lead owns batching, neither stages/commits. At each end invoke `measure finish` for its snapshot with returned child identity; relay compact paths/checks and repair/route cross-boundary needs unless `NEEDS_AUTHORITY`.
...
5. **Observe one full suite after rebase.** At `FULL_SUITE_SHA`, derive `EXPECTED_BASE_SHA`/`EXPECTED_HEAD_SHA`/`EXPECTED_DIFF_SHA256`; invoke `measure start --label "Full suite"` and `measure start --label "Create PR"`. In parallel start full-suite lane and `Skill(spectre-create_pr)` pending with target, `--orchestrated --pr-phase pending`, work ID, and tuple; it returns a draft with local verification `RUNNING`. At each end `measure finish` with child identity when available. No duplicate suites or raw child output.
