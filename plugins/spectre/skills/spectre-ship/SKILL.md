...
1. **Resolve once.** Confirm feature branch, target, `FEATURE_ROOT`, set, and no unrelated/sensitive changes; stop on `main`/`master`. Reuse current-thread `CLEANED_THROUGH_SHA` only when unambiguous and ancestral. Invoke `spectre-workflow measure start --label Ship`; below, `measure` means `spectre-workflow measure`.
...
5. **Refresh work before the draft split.** Carry Execute's exact work ID/run or PR/candidate into `Skill(spectre-capture)` before either. Operandless/Ship-run misses never allocate. Save failure does not block draft; report recovery, not success.
