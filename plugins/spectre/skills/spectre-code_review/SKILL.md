...
2. **Use one external-first review route from repo root; do not probe alternatives.** Codex primary runs:
   `claude -p --model claude-opus-5-5 --effort high --permission-mode dontAsk --allowedTools "Read,Grep,Glob,LS,Bash(mkdir -p *),Bash(git diff *),Bash(git show *),Bash(git status *),Bash(git rev-parse *),Write" --output-format text "$REVIEW_PROMPT"`
   Claude primary runs:
   `codex exec -C "$PWD" -m gpt-6-sol -c 'model_reasoning_effort="high"' -s workspace-write "$REVIEW_PROMPT" < /dev/null`
   Record route metadata as `Claude Code|claude-opus-5-5|high|Codex -> Claude Code` or `Codex|gpt-6-sol|high|Claude Code -> Codex`. Keep a 20-minute launcher-side poll limit; do not pass duration guidance to the reviewer. Quiet output is not failure. The reviewer writes only `REVIEW_REPORT`. Primary semantic self-review is prohibited.
