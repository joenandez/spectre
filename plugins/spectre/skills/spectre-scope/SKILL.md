...
- Search the actual task with `spectre knowledge search <task> --project-dir .`; use previews within the shared budget, assess applicability, then exact-load applicable records only. Retain each used ID and revision in `concepts/scope.md` with the decision it informed, excluding preview-only and unloaded candidates.
