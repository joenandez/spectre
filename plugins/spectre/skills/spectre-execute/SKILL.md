...
5. **Capture boundaries.** After the exact run/work identity exists, invoke `Skill(spectre-work-record)` once at start with a minimal truthful intent/scope account. Workers return compact findings/evidence; after each accepted batch the primary invokes `Skill(spectre-capture)` only for qualifying reusable knowledge, otherwise continues. Invoke `Skill(spectre-work-record)` only for a meaningful blocked handoff or resolved resumption, never a task, batch, check, review, commit, or routine-progress update. At terminal completion invoke `Skill(spectre-work-record)` once with the truthful seven-section account: self records reached execution/verification/PR states; parent remains `IMPLEMENTATION_READY`/`ACCEPTANCE_PENDING`; failed/incomplete work is not completed. Capture failure reports recovery input and does not block verification, review, proof, lifecycle authority, or delivery.
