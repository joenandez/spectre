# Minimum Solution Selection

Select the smallest solution shape before detailed drafting. This reference is the canonical policy for Plan selection; renderers and reviewers consume its completed record rather than redefining it.

## Record

Persist `## Minimum Solution Selection` in existing `task_context.md`, bound to confirmed Scope/authority and accepted evidence. Record:
...
Plan obtains the observed route from this completed record before one selected-depth draft. Detailed rendering may add implementation detail, never undeclared owned complexity; genuine insufficiency returns here. Plan Review compares the resulting plan with this record. Existing standalone rendering uses this reference locally when no Plan record exists.
