...
3. Gather proportional evidence: XS/S local except probe; M ≤2 relevant `@spectre:finder`, `@spectre:analyst`, or `@spectre:patterns`; L/XL necessary dimensions. In the existing parallel research wave, M reserves one available evidence slot for a fresh evidence-only challenger only for durable state, identity, public contract, migration, dependency, or workflow/lifecycle; L/XL always reserve it. The challenger writes nothing; primary dispatches it with the wave, then persists accepted evidence/IDs/revisions in `task_context.md`. `@spectre:web-research` only decides external API/framework. Refine knowledge search after affected files are known; load a work body only for a stated question, never reload an unchanged revision, and pass workers compact findings with provenance.
