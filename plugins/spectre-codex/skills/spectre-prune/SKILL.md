...
- For non-trivial sets, the calling primary directly dispatches up to 4 read-only leaf `@spectre_analyst` agents by file/module. Return only evidenced `SAFE_TO_REMOVE`, `NEEDS_VALIDATION`, or `KEEP` verdicts in-thread.
