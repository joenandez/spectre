...
5. **Verification Ledger** — completed ids/commits/files; verified HEADs/check ids/results; assessment; Plan Review state (`not-required:<XS|S>|closed`) with applicable paths/hashes; finalized plan hash; derived pair paths/hashes; review route; repairs/failures; E2E status. No raw output or report prose.
