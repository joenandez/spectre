...
- Reuse current-request knowledge results/loads; otherwise follow Project knowledge routing. Refine only for an unresolved question or new subject. Exact-load applicable guidance before affected decisions. Keep only loaded IDs/revisions in existing `task_context.md`; previews and unloaded candidates are not evidence.
