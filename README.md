...
- Your Agent will suggest the most relevant next step in the SPECTRE workflow automatically. For example, `/spectre:spectre-ux` if user flows are important, or directly to the full `/spectre:spectre-plan` flow to create the implementation plan.

- When context window is full or you reach a natural stopping point, run`/spectre:spectre-handoff` to generate a clear summary of the current work, next steps, open Qs, etc. Then start a new session (`/clear`) and your handoff is automatically injected into the next session. You'll see a summary when you start.

  - subsequent `/spectre:spectre-handoff` requests will use a subagent to combine to maintain continuity.
  - `/spectre:spectre-forget` when you are switching gears and want to clear the handoff context.
...
- `execution_state.md` - execution progress tracker when using `/spectre:spectre-execute` with a plan SPECTRE did not generate
...
SPECTRE maintains and accumulates context across sessions when you use the /spectre:spectre-handoff command. To get the most from SPECTRE's Session Memory, we recommend that you:

1. run /spectre:spectre-handoff when you are switching gears or the context window is getting full, and

2. start a new session (`/clear` in Claude Code) so your Agent resumes from the saved context.

### How It Works

When you run /spectre:spectre-handoff, a status report will get generated for that session, and automatically loaded into your context window for the next session. You'll see a nice summary when you start the next session (`/clear` in Claude Code).
...
If you want to start fresh — /spectre:spectre-forget archives the current branch's active files under `.spectre/handoffs/{branch}/archive/`.
...
`/spectre:spectre-learn` is the on-demand route: use it to preserve an evidenced insight or correction, or to request a work summary. A bare invocation can truthfully return a no-op; it does not create a knowledge skill, a startup entry, or a mandatory post-work task.
